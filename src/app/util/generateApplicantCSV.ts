...
export async function generateApplicantCSV(data: Prisma.Hacker_Applications[]) {
  ...
  for (const applicant of data) {
    applicantsWithResume.push({
      ...applicant,
      resume_url: `https://shellhacks.net/api/hackers/${applicant.id}/resume`, //we cannot be generating a signed url for each applicant, its too expensive. Maybe some custom api route + kv store can take care of this??
