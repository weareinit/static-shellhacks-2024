...
export default function ChangeAvatarModal({
  ...
  isOpen: boolean;
  toggleOpen: () => void;
  handleDinoChange: (dino: number) => void;
}) {
  return isOpen
    ? createPortal(
        <div className="fixed inset-0 z-10 flex h-screen w-screen flex-col items-center justify-center bg-black bg-opacity-80">
          <div className="min-h-screen overflow-y-scroll bg-white p-3 md:min-h-0 md:w-[90%] md:rounded-lg md:p-5">
            ...
            <div className="flex flex-row flex-wrap justify-center gap-3 md:justify-start md:gap-8">
              {dinosaurNames.map((dino, index) => (
                <Image
                  src={`/assets/new/dinosaurs/${dino}.svg`}
                  width={110}
                  height={110}
                  key={dino}
                  alt={dino}
